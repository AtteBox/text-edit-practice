...
function generateMockHighscores(count: number = 105) {
 ...
 test.describe("Highscores page", () => {
  ...
  test("pagination navigation works correctly", async ({ page }) => {
    ...
    await expect(page.getByRole("button", { name: "1" })).toHaveClass(/bg-violet-600/);
    ...
    await expect(page.getByRole("button", { name: "2" })).toHaveClass(/bg-violet-600/);
    await expect(page.getByRole("button", { name: "1" })).not.toHaveClass(/bg-violet-600/);
    ...
    await expect(page.getByRole("button", { name: "3" })).toHaveClass(/bg-violet-600/);
  ...
  test("last page shows correct item count", async ({ page }) => {
    ...
    await expect(page.getByText("Showing 101-105 of 105 entries")).toBeVisible();
  ...
  test("displays empty state when no highscores", async ({ page }) => {
    ...
    await expect(page.getByText("No highscores yet. Be the first to play!")).toBeVisible();
